...
%DIF DEL /Users/bb/Downloads/teff.tex   Fri Sep 25 12:55:23 2020
%DIF ADD teff.tex                       Fri Sep 25 12:53:19 2020
\usepackage{amsmath}
\usepackage{amssymb}
%\usepackage{widetext}
\usepackage{verbatim}
...
\DIFdelbegin \subsection{\DIFdel{Evaluation of conformational dynamics of lipids against NMR data}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection*{\DIFadd{Evaluation of lipid conformational dynamics against NMR data.}}\DIFaddend \label{sec:theory}
...
\end{equation}
where \DIFaddbegin \DIFadd{angular brackets depict averaging over time,
}\DIFaddend $\vec{\mu}(t)$ is the unit vector in the direction of the C--H bond at time $t$\DIFaddbegin \DIFadd{, }\DIFaddend and $P_{2}$ is the second order Legendre polynomial \DIFdelbegin \DIFdel{.
The angular brackets depict averaging over time.
}\DIFdelend \DIFaddbegin \DIFadd{$P_2(x)=\frac{1}{2}(3x^2-1)$.
}\DIFaddend For lipids in a bilayer, the internal dynamics and rotation around membrane normal
...
\DIFdelbegin \DIFdel{This order parameter can be }\DIFdelend \DIFaddbegin \DIFadd{$S_\mathrm{CH}$ can be independently }\DIFaddend measured using dipolar coupling in $^{13}$C NMR or quadrupolar coupling in $^{2}$H NMR \DIFaddbegin \DIFadd{experiments}\DIFaddend ,
and is highly useful in order to evaluate conformational ensembles of lipids \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{ollila16}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Ollila:2016a}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
%, are measured in NMR experiments from this plateau.
%DIF < As $S_{\rm{CH}}$ describes the conformational ensemble of the molecule, the fast-decaying component of the rotational correlation function intuitively depicts the time needed to sample these conformations. 

In order to analyze the internal dynamics of lipids, 
the C-H bond autocorrelation function is often \DIFdelbegin \DIFdel{expressed as %DIF < the product of two functions
}\DIFdelend \DIFaddbegin \DIFadd{written as a product %DIF > of two functions
}\DIFaddend \begin{equation}
g(\tau)=g_{\mathrm{f}}(\tau)g_{\mathrm{s}}(\tau) ,
\end{equation}
where $g_{\mathrm{f}}(\tau)$ \DIFdelbegin \DIFdel{represents the fast dynamics below $\sim \mu$s timescales, %DIF < from lipid conformational dynamics,
%DIF < characterizes fast decays owing to, for example, the molecular  rotations,
}\DIFdelend \DIFaddbegin \DIFadd{characterizes fast decays owing to, for example, the molecular  rotations, }\DIFaddend and $g_{\mathrm{s}}(\tau)$ describes slow \DIFdelbegin \DIFdel{decay }\DIFdelend \DIFaddbegin \DIFadd{decays that originate }\DIFaddend from, e.g., lipid diffusion between lamellae with different orientations\DIFdelbegin \DIFdel{and
}\DIFdelend \DIFaddbegin \DIFadd{, and also periodic motions due to the use of }\DIFaddend magic angle spinning \DIFdelbegin \DIFdel{in solid state NMR experiments }\DIFdelend \DIFaddbegin \DIFadd{conditions }\DIFaddend (Fig.~\ref{fig:schem_teff}).
%DIF < The two components, along with the oscillation due to magic angle spinning at the $\sim$kHz region, are depicted in Fig.~\ref{fig:schem_teff}.
\DIFdelbegin \DIFdel{In multilamellar POPC samples at 300 K in liquid crystalline state, the correlation time of 4.2 ms was estimated for $g_{\mathrm{s}}(\tau)$, whereas $g_{\mathrm{f}}(\tau)$ decays to a plateau value  within a few hundred nanoseconds~\mbox{%DIFAUXCMD
\cite{ferreira15}}\hspace{0pt}%DIFAUXCMD
.
%DIF < The C--H bond order parameters
%DIF < The characteristic time can be quantified via the effective correlation time
}\DIFdelend \DIFaddbegin \DIFadd{Ferreira et al.~\mbox{%DIFAUXCMD
\cite{ferreira15} }\hspace{0pt}%DIFAUXCMD
...
\DIFadd{Now, as $S_{\rm{CH}}$ describe the conformational ensemble of the molecule, the fast-decaying component of the rotational correlation function intuitively depicts the time needed to sample these conformations.
}\DIFaddend The internal dynamics containing multiple timescales can be quantified using the effective correlation time
\begin{equation}
\label{eq:teff}
...
The integrand \DIFdelbegin \DIFdel{is Eq.\ref{eq:teff} }\DIFdelend \DIFaddbegin \DIFadd{in Eq.~}\eqref{eq:teff} \DIFaddend defines the reduced and normalized correlation function
\begin{equation}
\label{eq:nBCF}
...
%DIF < It is easily seen that in the presence of more long-lived correlations $\tau_\mathrm{e}$ grows, signaling that more time is needed for full conformational sampling.  
\DIFaddbegin \DIFadd{It is easily seen that in the presence of more long-lived correlations $\tau_\mathrm{e}$ grows, signaling that more time is needed for full conformational sampling.}\todo{Redundant sentence?}
\DIFaddend 

\begin{figure}[t]
\includegraphics[scale=0.45]{./figures/gfun_draft.pdf} 
\caption{\DIFdelbeginFL \DIFdelFL{The }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Illustration of the }\DIFaddendFL autocorrelation function $g(\tau)$ a) The fast mode (white background) and the slow mode (shaded green) of the correlation function along with the oscillation owing to magic angle spinning. The fast mode decays to a plateau \DIFdelbeginFL \DIFdelFL{quantifying the $S_{\mathrm{CH}}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{on which $g(\tau)=S^2_{\mathrm{CH}}$ }\DIFaddendFL while the slow mode gives the final descent to zero. b) Illustration of typical C--H bond autocorrelation function obtained from a MD simulation. The gray area under the curve \DIFdelbeginFL \DIFdelFL{gives a way of quantifying the $\tau_\mathrm{e}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{is equal to $(1-S^2_{\mathrm{CH}})\tau_\mathrm{e}$}\DIFaddendFL . }
...
trajectory with the experimental data \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{feller02,wohlert06,klauda08,klauda08II,ferreira15,ollila16}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
from experiments \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{??}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Roberts:2009a,leftin11}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .

Here, we \DIFdelbegin \DIFdel{use use two parameters:
the effective correlation times, }\DIFdelend \DIFaddbegin \DIFadd{analyze lipid dynamics based on two quantities that are readily available
...
The effective C--H bond correlation times }\DIFaddend $\tau_\mathrm{e}$, and
\DIFdelbegin \DIFdel{$R_1$ spin relaxation rates from $^{13}$C-NMR experiments~\mbox{%DIFAUXCMD
\cite{ferreira15,pham15,Volke:1995a}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{the spin-lattice relaxation rates $R_1$.
}

\DIFaddend %We analyze lipid dynamics based on two quantities available from published $^{13}$C-NMR experiments:~\cite{ferreira15,pham15,Volke:1995a}
...
%directly quantifiable from atomistic MD simulations.
\DIFdelbegin \DIFdel{The effective correlation times }\DIFdelend \DIFaddbegin \DIFadd{As discussed above (Eq.~}\eqref{eq:teff}\DIFadd{), the $\tau_\mathrm{e}$ }\DIFaddend detect essentially an average over \DIFaddbegin \DIFadd{all }\DIFaddend the time scales relevant for the lipid internal dynamics, and
have \DIFaddbegin \DIFadd{an }\DIFaddend intuitive relation to dynamics\DIFdelbegin \DIFdel{as larger }\DIFdelend \DIFaddbegin \DIFadd{: Larger }\DIFaddend values always indicate \DIFdelbegin \DIFdel{slower dynamics}\DIFdelend \DIFaddbegin \DIFadd{a slowdown}\DIFaddend .  
%Most importantly, i
...
\DIFdelbegin \DIFdel{On the other hand, }\DIFdelend \DIFaddbegin 

\DIFadd{In contrast, it is impossible to directly connect an increase/decrease of }\DIFaddend $R_1$ \DIFdelbegin \DIFdel{values are most }\DIFdelend \DIFaddbegin \DIFadd{to speedup/slowdown of specific motions.
This is because $R_1$ values are only }\DIFaddend sensitive to processes \DIFdelbegin \DIFdel{with the timescales around $\sim$1--10~ns with typical
%DIF < in a rather narrow time scale window set by the
magnetic field strengths.
%DIF <  frequency,
}\DIFdelend \DIFaddbegin \DIFadd{whose timescales fall within a certain window:
Smaller $R_1$ only tells that there are less processes occurring in this window.
}\DIFaddend %The spin-lattice relaxation rate $R_1$ defines the time-scale on which $^{13}$C longitudal magnetization equilibrates.
\DIFaddbegin \DIFadd{More specifically,
}\DIFaddend $R_1$ is related to molecular dynamics through
\DIFdelbegin \DIFdel{equation
...
\end{split}
\end{align}
where $N_{\mathrm{H}}$ is the number of bound hydrogens, $\omega_{\mathrm{H}}$ and $\omega_{\mathrm{C}}$ \DIFdelbegin \DIFdel{are the Larmor frequencies
for $^{1}$}\DIFdelend \DIFaddbegin \DIFadd{the $^1$}\DIFaddend H and $^{13}$C \DIFaddbegin \DIFadd{NMR Larmor frequencies}\DIFaddend , and $d_{\mathrm{CH}}$ \DIFdelbegin \DIFdel{is }\DIFdelend the rigid dipolar coupling constant. For the methylene bond, $d_{\mathrm{CH}}/2\pi$ approximately equals to -22~kHz.
\DIFdelbegin %DIFDELCMD < \todo{why there is a minus sign above?}
%DIFDELCMD < %%%
\DIFdelend %DIF > \todo{Why there is a minus sign above? TMF: because the horns of the Pake pattern come from the 90 degree orientation of the rigid C-H bond and P2(cos(90)) is negative. this is the convention in NMR simulation programs. not an important detail and skipping the negative sign is also fine.}
...
The spectral density \DIFaddbegin \DIFadd{$j(\omega)$ }\DIFaddend is the Fourier transformation
%DIF > depicting the $^{13}$C relaxation rates (at frequency $\omega$) is expressed as
\DIFaddbegin \begin{equation}
\DIFadd{j}{\DIFadd{(\omega)}}\DIFadd{=2\int_{0}^{\infty}\cos(\omega\tau)g(\tau)\mathrm d\tau
}\end{equation}
\DIFaddend of the rotational correlation function %DIF < depicting the $^{13}$C relaxation rates (at frequency $\omega$) is expressed as
\DIFdelbegin \begin{displaymath}
\DIFdel{j}{\DIFdel{(\omega)}}\DIFdel{=2\int_{0}^{\infty}\cos(\omega\tau)g(\tau)\mathrm d\tau.
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{$g(\tau)$ defined in Eq.~}\eqref{eq:BCF}\DIFadd{.
}\DIFaddend %which is 
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Clearly, the }\DIFaddend connection between $R_1$ and molecular dynamics is not straightforward\DIFdelbegin \DIFdel{,
but
}\DIFdelend \DIFaddbegin \DIFadd{;
}\DIFaddend %are not intuitively related to changes in process speeds: A
\DIFaddbegin \DIFadd{however, the }\DIFaddend magnitude of $R_1$ \DIFdelbegin \DIFdel{reflects }\DIFdelend \DIFaddbegin \DIFadd{does reflect }\DIFaddend the relative significance of \DIFdelbegin \DIFdel{the }\DIFdelend processes
...
with \DIFdelbegin \DIFdel{time-scales }\DIFdelend \DIFaddbegin \DIFadd{timescales }\DIFaddend near the inverse of \DIFdelbegin \DIFdel{proton and $^{13}$C Larmor frequencies (typically
}\DIFdelend \DIFaddbegin \DIFadd{$\omega_{\mathrm{H}}$ and $\omega_{\mathrm{C}}$.
These depend on the field strength used in the NMR experiments; typically
$R_1$ is most sensitive to motions with time scales }\DIFaddend $\sim$1--10~ns\DIFdelbegin \DIFdel{).
%DIF < Since the Larmor frequencies depend on the field strength used in the NMR measurements, this
%DIF < typically makes $R_{1}$
%DIF < sensitive to
%DIF < Importantly, a change in $R_{1}$ thus indicates a difference in the relative
%DIF < amounts of processes within the detection window, and therefore does not give information on the modulation of the total sampling rate.  
}\DIFdelend \DIFaddbegin \DIFadd{.
...
\DIFdelbegin \DIFdel{subsection}%DIFDELCMD < {%%%
\DIFdel{Experimental data acquisition and analysis}%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \subsection*{\DIFadd{Experimental data acquisition and analysis.}}
\DIFaddend %
All the experimental quantities were collected from the literature sources referred at the respective figures.   

\DIFdelbegin \subsection{\DIFdel{Simulational data acquisition and analysis}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection*{\DIFadd{Simulational data acquisition and analysis.}}
\DIFaddend %
...
intact even with 50\% of cholesterol, supporting the idea~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{citetoarcihved} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Antila:2020a} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend that acyl chains and headgroup can respond
almost independently on even large membrane distractions.
In line with general picture from order parameters \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{ollila16}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Ollila:2016a}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , MD simulations capture the changes
...
\label{fig:hydration}
\DIFdelbeginFL %DIFDELCMD < \todo{add ref to archive for full hydration POPC data}\\
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \bibliography{lipids,ff,simu,pdb,journals} %%%
\DIFdelend \DIFaddbegin \bibliography{lipids,ff,simu,pdb,journals,technical} \DIFaddend %,bbRefs}
