...
%DIF DEL /Users/bb/Downloads/teff.tex   Fri Sep 25 14:06:49 2020
%DIF ADD teff.tex                       Fri Sep 25 13:40:59 2020
\usepackage{amsmath}
\usepackage{amssymb}
%\usepackage{widetext}
\usepackage{verbatim}
...
\clearpage
\section{Methods}
\subsection*{Evaluation of lipid conformational dynamics \DIFaddbegin \DIFadd{in MD }\DIFaddend against NMR data.}\label{sec:theory}
\DIFaddbegin \DIFadd{The internal dynamics of lipids in MD simulations has been previously evaluated by comparing
the $^2$H or $^{13}$C spin relaxation times or effective correlation times calculated from simulation
trajectory with the experimental data \mbox{%DIFAUXCMD
\cite{feller02,wohlert06,klauda08,klauda08II,ferreira15,Ollila:2016a}}\hspace{0pt}%DIFAUXCMD
.
...
from experiments \mbox{%DIFAUXCMD
\cite{Roberts:2009a,leftin11}}\hspace{0pt}%DIFAUXCMD
.
%DIF > 
Here, we analyze lipid dynamics based on two quantities that are readily available
from published~\mbox{%DIFAUXCMD
\cite{ferreira15,pham15,Volke:1995a,Antila:2020a} }\hspace{0pt}%DIFAUXCMD
$^{13}$C-NMR experiments and
...
\subsubsection*{\DIFadd{Effective C--H bond correlation times $\tau_\mathrm{e}$.}}
\DIFaddend In a lipid bilayer in liquid crystalline phase, each individual lipid molecule samples an internal conformational
ensemble and rotates around
%the symmetry axis of the bilayer which coincides with
the membrane normal direction.
The conformational dynamics of a lipid molecule can be characterized using
the second order autocorrelation functions of C--H bonds %at time $\tau$
\begin{equation}
\label{eq:BCF}
...
have timescales below $\mu$s, leading to \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend decay of the correlation function to a non-zero plateau value (Fig.~\ref{fig:schem_teff}).
...
%, are measured in NMR experiments from this plateau.

In order to analyze the internal dynamics of lipids, 
the C-H bond autocorrelation function \DIFaddbegin \DIFadd{(Eq.~}\eqref{eq:BCF}\DIFadd{) }\DIFaddend is often written as a product %of two functions
\begin{equation}
g(\tau)=g_{\mathrm{f}}(\tau)g_{\mathrm{s}}(\tau) ,
\end{equation}
where $g_{\mathrm{f}}(\tau)$ characterizes fast decays owing to, \DIFdelbegin \DIFdel{for example}\DIFdelend \DIFaddbegin \DIFadd{e.g.}\DIFaddend , the molecular  rotations, and $g_{\mathrm{s}}(\tau)$ describes slow decays that originate from, e.g., lipid diffusion between lamellae with different orientations, and \DIFdelbegin \DIFdel{also }\DIFdelend periodic motions due to the use of magic angle spinning conditions (Fig.~\ref{fig:schem_teff}).
Ferreira et al.~\cite{ferreira15} have \DIFdelbegin \DIFdel{shown }\DIFdelend \DIFaddbegin \DIFadd{experimentally demonstrated }\DIFaddend that for all phospholipid carbons
\DIFdelbegin \DIFdel{the spin-lattice relaxation rate in the rotating frame, $R_{1\rho}$, becomes constant above a spin lock frequency of 50 kHz indicating that the }\DIFdelend %DIF > the spin-lattice relaxation rate in the rotating frame, $R_{1\rho}$, becomes constant above a spin lock frequency of 50 kHz indicating that
\DIFaddbegin \DIFadd{the }\DIFaddend motion correlation times contributing to $g_{\mathrm{f}}$ and $g_{\mathrm{s}}$ are well below $\mu$s and well above 100 $\mu$s, respectively.
This separation of time-scales gives rise to the plateau $g(1\,\mu\mathrm s  \lesssim \tau \lesssim 100\,\mu\mathrm s)=S^2_\mathrm{CH}$ \DIFdelbegin \DIFdel{observed }\DIFdelend \DIFaddbegin \DIFadd{illustrated }\DIFaddend in Fig.~\ref{fig:schem_teff}.

Now, as $S_{\rm{CH}}$ describe the conformational ensemble of the molecule, the fast-decaying component of the rotational correlation function intuitively \DIFdelbegin \DIFdel{depicts }\DIFdelend \DIFaddbegin \DIFadd{reflects }\DIFaddend the time needed to sample these conformations.
...
\end{equation}
which is related to the \DIFdelbegin \DIFdel{integrated }\DIFdelend \DIFaddbegin \DIFadd{gray-shaded }\DIFaddend area below the correlation function in Fig.~\ref{fig:schem_teff}\DIFdelbegin \DIFdel{B}\DIFdelend .
...
\DIFdelbegin %DIFDELCMD < \todo{Maybe also add 1C that explicitly shows $g'_{\mathrm{f}}$?}
%DIFDELCMD < %%%
\DIFdelend It is easily seen that in the presence of more long-lived correlations $\tau_\mathrm{e}$ grows, signaling that more time is needed for full conformational sampling.\todo{Redundant sentence?}
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \todo{Maybe also add 1C that explicitly shows $g'_{\mathrm{f}}$?}
\DIFaddendFL \end{figure} 



\DIFdelbegin \DIFdel{The internal dynamics of lipids in MD simulations has been previously evaluated by comparing
the $^2$H or $^{13}$C spin relaxation times or effective correlation times calculated from simulation
trajectory with the experimental data \mbox{%DIFAUXCMD
\cite{feller02,wohlert06,klauda08,klauda08II,ferreira15,Ollila:2016a}}\hspace{0pt}%DIFAUXCMD
.
...
%DIFDELCMD < %%%
\DIFdel{Here, we analyze lipid dynamics based on two quantities that are readily available
from published~\mbox{%DIFAUXCMD
\cite{ferreira15,pham15,Volke:1995a,Antila:2020a} }\hspace{0pt}%DIFAUXCMD
$^{13}$C-NMR experiments and
directly quantifiable from atomistic MD simulations:
...
%DIFDELCMD < %%%
%DIF < We analyze lipid dynamics based on two quantities available from published $^{13}$C-NMR experiments:~\cite{ferreira15,pham15,Volke:1995a}
%DIF < The effective C--H bond correlation time $\tau_\mathrm{e}$ and
%DIF < the spin-lattice relaxation rate $R_{1}$,
%DIF < both %experimentally available through NMR measurements, and
%DIF < directly quantifiable from atomistic MD simulations.
\DIFdelend \DIFaddbegin \subsubsection*{\DIFadd{Spin-lattice relaxation rates $R_1$.}}
\DIFaddend As discussed above (Eq.~\eqref{eq:teff}), the $\tau_\mathrm{e}$ detect essentially an average over all the time scales relevant for the lipid internal dynamics, and
have an intuitive relation to dynamics: Larger values always indicate a slowdown.  
%Most importantly, i
%Increasing $\tau_\mathrm{e}$ always signals some type of slowdown in the C--H bond dynamics \cite{ferreira15}.
%, making the interpretation less ambiguous than for $R_1$, where slowdown in the dynamics can lead to either an increase or a decrease of $R_{1}$ value
%
%
...
%The spin-lattice relaxation rate $R_1$ defines the time-scale on which $^{13}$C longitudal magnetization equilibrates.
More specifically,
...
%are not intuitively related to changes in process speeds: A
however, the magnitude of $R_1$ does reflect the relative significance of processes
%in the sensitive time window decreases,
%but not if the corresponding processes become faster or slower.
...
%The dependency of $R_{1}$ on the spectral densities $j$ at the Larmor frequencies means that the $R_{1}$ value depicts
%the
%relative amounts of relaxation processes
...
\todo{Markus: look at the new figure, CHARMM36 error estimates pose difficulties here, change C36 data?}
All the force fields investigated qualitatively reproduce the increase in $\tau_\mathrm e$ (see Fig.~\ref{fig:chol}B):
Slipids gives the best magnitude estimates, while CHARMM36 and MacRog clearly overestimate the changes at the glycerol, C2, and C3 carbons. Notably, Macrog erroneously predict slow down also for the headgroup $\beta$ and $\alpha$ carbons, for which experiments detect no change. \todo{see new error bars for Macrog}
%
...
\DIFdelbegin \subsection*{\DIFdel{Effect of dehydration.}}
%DIFAUXCMD
\DIFdel{Understanding the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend impact of dehydration on the structure and dynamics of lipid bilayers is of considerable \DIFdelbegin \DIFdel{biological interest . Most prominently, }\DIFdelend \DIFaddbegin \DIFadd{interest as  }\DIFaddend membrane fusion is always preceeded by removal of water between the opposing \DIFdelbegin \DIFdel{membrane and dehydration therefore may considerably affect the fusion characteristics such as the rate }\DIFdelend \DIFaddbegin \DIFadd{membranes and therefore may affect the rate of fusion}\DIFaddend . Lipid bilayers in dehydrated states are also found, e.g., in skin tissue. 

Figure~\ref{fig:hydration}A shows how a mild dehydration \DIFdelbegin \DIFdel{alters the
}\DIFdelend \DIFaddbegin \DIFadd{affects
}\DIFaddend C--H bond dynamics in the PC headgroup and glycerol backbone;
...
\caption{Effect of \DIFdelbeginFL \DIFdelFL{dehydration }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{drying }\DIFaddendFL on %the
...
\DIFdelbeginFL \DIFdelFL{PC headgroup }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{POPC }\DIFaddendFL at decreasing hydration in three MD models.
\DIFaddbeginFL \DIFaddFL{Note that three Berger data points (4, 12, 16, and 24~w/l) are from DLPC bilayers.
}\DIFaddendFL Symbols give the mean of segment hydrogens,
...
in experiments and simulations, with experiments indicating an increasing trend upon dehydration. Experimental POPC data at 28 w/l is from \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
\cite{reftoarchive} }\hspace{0pt}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Ref.~\mbox{%DIFAUXCMD
\citenum{Antila:2020a} }\hspace{0pt}%DIFAUXCMD
}\DIFaddendFL (filled square), POPC at 20 and 5\,w/l from Ref.~\citenum{Volke:1995a} (diamond), and DMPC data at 13 w/l from Ref.~\citenum{pham15} (hollow square). 
See Table~\ref{tab:hydr} for simulation details.\DIFdelbeginFL \DIFdelFL{Note that three Berger data points (4, 12, 16, and 24~w/l) are from DLPC bilayers, while all the rest are from POPC simulations.}\DIFdelendFL }
\label{fig:hydration}
\end{figure}

Figure~\ref{fig:hydration}B shows the effects of dehydration \DIFdelbegin \DIFdel{on $\tau_\mathrm e$ }\DIFdelend in three MD models.
Combination of
...
Owing to the uncertainties, we only point out the qualitative trends. For all carbons in the headgroup and glycerol segments, the simulated  $\tau_\mathrm e$ indicates slow down upon dehydration. This \DIFdelbegin \DIFdel{manifests as }\DIFdelend \DIFaddbegin \DIFadd{is manifested in }\DIFaddend the increase in the magnitude of the error estimate (cf. the Berger data for $\beta$ and $\alpha$) as well as \DIFaddbegin \DIFadd{in }\DIFaddend the increase of the lower limit of the error. 
...
experiments reducing hydration levels below 10\,w/l would also \DIFdelbegin \DIFdel{detect }\DIFdelend \DIFaddbegin \DIFadd{show }\DIFaddend an increase in $\tau_\mathrm e$.
...
\DIFdelbegin \DIFdel{simulated as well as 
}\DIFdelend a collection of experimental $^{13}$C-NMR $R_1$ rates
measured at 125\,MHz for the headgroup segments
at different water contents;
\DIFdelbegin %DIFDELCMD < \todo{do we really need to specify the experiments here since they are now at the caption too}
%DIFDELCMD < %%%
\DIFdelend in addition to the full hydration POPC data from Fig.~\ref{fig:teff_R1},
\DIFdelbegin \DIFdel{experimental results for }\DIFdelend DMPC at 13\,w/l~\cite{pham15}, % 125 MHz, 300K
...
%
While no force field \DIFdelbegin \DIFdel{perfectly }\DIFdelend reproduces all the experimental data,
...
well \DIFdelbegin \DIFdel{represent }\DIFdelend \DIFaddbegin \DIFadd{reproduce }\DIFaddend the experimental conformational ensemble~\cite{botan15}.
%
...
%DIF < \todo{Make the point that the 500-ns simulations indicated by Vogel~\cite{vogel12} are not needed for sufficient sampling?}
\DIFaddbegin \todo{Make the point that the 500-ns simulations indicated by Vogel~\cite{vogel12} are not needed for sufficient sampling?}
\DIFaddend 

In addition to standard conditions, we explored how the dynamics react to addition of cholesterol \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{or NaCl, or to }\DIFaddend removal of water.
%
MD qualitatively captures \DIFdelbegin \DIFdel{the slow-down of }\DIFdelend \DIFaddbegin \DIFadd{that when cholesterol is mixed into a POPC bilayer, the }\DIFaddend conformational dynamics in the tail and glycerol regions \DIFdelbegin \DIFdel{when cholesterol is mixed into a POPC bilayer}\DIFdelend \DIFaddbegin \DIFadd{slows down}\DIFaddend ; however, some force fields \DIFdelbegin \DIFdel{erroneously predict changes also in the short-scale dynamics of the headgroup carbons }\DIFdelend \DIFaddbegin \DIFadd{predict an (erroneous) slowdown also for the headgroup }\DIFaddend (Fig.~\ref{fig:chol}).
%
\DIFdelbegin \DIFdel{Upon }\DIFdelend \DIFaddbegin \DIFadd{With increasing NaCl concentration, a behaviour reminiscent of the molecular electrometer was observed: Amount of ion binding to the bilayer correlated with the magnitude increase in $\tau_\mathrm e$; this could open up the possibility of using $\tau_\mathrm e$ in quantifying cation binding to lipid bilayers.
%DIF > 
When }\DIFaddend reducing the water content, MD exhibits \DIFdelbegin \DIFdel{slow-down }\DIFdelend \DIFaddbegin \DIFadd{slowdown }\DIFaddend of headgroup and backbone dynamics below $\sim$10 waters per lipid in qualitative agreement with experimental data. \DIFdelbegin \DIFdel{That said, only CHARMM36 qualitatively captures the experimentally detected increase of $R_1$ rates upon dehydration.  
}\DIFdelend \DIFaddbegin \todo{ Hydration needs some kind of statement of significance.}
\DIFaddend 
...
Importantly, this work demonstrates the \DIFdelbegin \DIFdel{potential }\DIFdelend \DIFaddbegin \DIFadd{power }\DIFaddend of open data in \DIFdelbegin \DIFdel{achieving this }\DIFdelend \DIFaddbegin \DIFadd{creating new knowledge out of existing trajectories }\DIFaddend at a reduced computational and labor cost. \DIFdelbegin \DIFdel{A pool of }\DIFdelend %DIF > Although no new simulations were performed for the purpose of this work, we were able to conduct a comprehensive study on the dynamics of MD models under several conditions. An interesting extension would be exploring other lipid headgroups individually as well as performing a comparison of MD model dynamics between headgroup types, as the available simulation data goes well beyond simulations of lipids with the phosphocholine headgroup.
\DIFaddbegin \DIFadd{If the data are }\DIFaddend well indexed and documented\DIFdelbegin \DIFdel{data provides an ideal platform for automation, and thus }\DIFdelend \DIFaddbegin \DIFadd{, this process could be automated and }\DIFaddend has the potential to facilitate faster progress\DIFdelbegin \DIFdel{in pinpointing the typical failures of existing models, in identifying key differences in models describing chemical variations under the same molecule type (eg. different lipid headgroups), and developing better models }\DIFdelend \DIFaddbegin \DIFadd{, e.g., in the development of MD force fields, for example }\DIFaddend through machine learning approaches.


%DIF < Importantly, this work demonstrates the power of open data in creating new knowledge out of existing trajectories at a reduced computational and labor cost. Although no new simulations were performed for the purpose of this work, we were able to conduct a comprehensive study on the dynamics of MD models under several conditions. An interesting extension would be exploring other lipid headgroups individually as well as performing a comparison of MD model dynamics between headgroup types, as the available simulation data goes well beyond simulations of lipids with the phosphocholine headgroup.
%DIF < If the data are well indexed and documented, this process could be automated and has the potential to facilitate faster progress, e.g., in the development of MD force fields, for example through machine learning approaches.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \acknowledgement
  \DIFdelbegin \DIFdel{H. A gratefully acknowledges the support from Osk. Huttunen's foundation, Finnish Academy of Science and Letters (Foundations’ Post Doc Pool), and the AvH Foundation.}%DIFDELCMD < \todo{do we need acknowledgement of computational resources? I ran everything on hot login node} 
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{This material is based upon work supported by XXX under Grant No. XXX. The project is/isn't part of the NMRlipids open collaboration (nmrlipids.blogspot.fi)
}\DIFaddend 
