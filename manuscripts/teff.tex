...
%As $S_{\rm{CH}}$ describes the conformational ensemble of the molecule, the fast-decaying component of the rotational correlation function intuitively depicts the time needed to sample these conformations. 

In order to analyze the internal dynamics of lipids, 
the C-H bond autocorrelation function is often expressed as %the product of two functions
\begin{equation}
g(\tau)=g_{\mathrm{f}}(\tau)g_{\mathrm{s}}(\tau) ,
\end{equation} 
where $g_{\mathrm{f}}(\tau)$ represents the fast dynamics below $\sim \mu$s timescales, %from lipid conformational dynamics,
%characterizes fast decays owing to, for example, the molecular  rotations,
...
%The two components, along with the oscillation due to magic angle spinning at the $\sim$kHz region, are depicted in Fig.~\ref{fig:schem_teff}.
...
%The C--H bond order parameters
%The characteristic time can be quantified via the effective correlation time
...
%in a rather narrow time scale window set by the
magnetic field strengths. % frequency,
...
%Since the Larmor frequencies depend on the field strength used in the NMR measurements, this
%typically makes $R_{1}$
%sensitive to
%Importantly, a change in $R_{1}$ thus indicates a difference in the relative
%amounts of processes within the detection window, and therefore does not give information on the modulation of the total sampling rate.  
...
\caption{Effect of dehydration on %the
...
in experiments and simulations, with experiments indicating an increasing trend upon dehydration. Experimental POPC data at 28 w/l is from \cite{reftoarchive} (filled square), POPC at 20 and 5\,w/l from Ref.~\citenum{Volke:1995a} (diamond), and DMPC data at 13 w/l from Ref.~\citenum{pham15} (hollow square). 
...
\label{fig:hydration}
\todo{add ref to archive for full hydration POPC data}\\
\end{figure}
...
\todo{do we really need to specify the experiments here since they are now at the caption too}
in addition to the full hydration POPC data from Fig.~\ref{fig:teff_R1},
experimental results for DMPC at 13\,w/l~\cite{pham15}, % 125 MHz, 300K
...
\acknowledgement
H.A gratefully acknowledges the support from Osk. Huttunen's foundation, Finnish Academy of Science and Letters (Foundations’ Post Doc Pool), and the AvH Foundation. \todo{do we need acknowledgement of computational resources? I ran everything on hot login node} 
