...
\begin{abstract}
Molecular dynamics (MD) simulations are widely used to monitor time-resolved motions of biomacromolecules, although it often remains unknown how closely the conformational dynamics %in MD
...
We found none of the tested MD models to fully reproduce the conformational dynamics. That said, the dynamics in CHARMM36 and Slipids are more realistic than in the Amber Lipid14, OPLS-based MacRog, and GROMOS-based Berger force fields, whose sampling of the glycerol backbone conformations is too slow. The %(qualitatively) good
...
%
%Slipids is the most realistic model to study headgroup dynamics.
%
%Slipids is the most realistic model to study changes headgroup dynamics caused by cholesterol.

%Not a single new simulation was run for the present work. This demonstrates the power of open-access MD trajectory databanks in benchmarking the available force fields---an indispensable first step in any serious MD study
...
Benchmarking the %suitability of
available force fields. %to the task at hand
...
%\todo{Tiago: We need a punchline highlighting that the results and findings obtained are extremely important for the lipid MD simulation community and that they were only possible due to the open data}
\end{abstract}
...
% disordered proteins and lipids composing cellular membranes
...
%
% but does not give information on the direction (speedup/slowdown) to which the processes changed.
%
% Tiago 3.11.: A simple measurement changing T would clarify this, so it is not a big deal as sort of emphasised.
% I would change the sentence after comma to:
%
% but does not give information on motions with time-scales outside this range.
%
...
%
Figure~\ref{fig:systemSize} demonstrates that $\tau_\mathrm e$ and $R_1$ do not systematically depend on the simulation size.
...
\begin{figure}[t]
	...
	Ref.\,\citenum{VenableSIMS}~\cite{Venable:2015a} (blue, system sizes 72 and 648 lipids), and
	Ref.\,\citenum{JavanainenSIMS} (red, system sizes 200, 800, and 1800 lipids).
...
Berger-POPC-07~\cite{ollila07a}/SPC\cite{berendsen81} %\cite{berger97,bachar04}
...
Lipid14 \cite{dickson14}/TIP3P\cite{Jorgensen:1983a}
...
ECC~\cite{melcr18}/SPC-E\cite{berendsen87}
...
The lack of coupling of the head group and chains of DPPC was demonstrated in simulations published by Klauda et al Biophysical Journal, 94, 3074-3083 (2008).
...
in experiments and simulations: Experiments indicate an increasing trend upon dehydration. Experimental POPC ($T=298$\,K) data at 28 w/l is from Ref.~\citenum{Antila:2020a} ($\blacksquare$), POPC (298\,K) at 20 and 5\,w/l from Ref.~\citenum{Volke:1995a} ($\blacklozenge$), and DMPC (303\,K) at 13 w/l from Ref.~\citenum{pham15} ($\square$). 
...
% as it almost reproduces the experimental conformational ensemble~\cite{botan15}.
%
...
%describes well the headgroup conformational ensemble~\cite{botan15},
...
\bibliography{lipids,ff,simu,pdb,journals,technical}
